...
export default function PngToJpgPage() {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        try {
          ...
          // Fill with solid white background to handle transparency
          ctx.fillStyle = '#FFFFFF';
          ctx.fillRect(0, 0, canvas.width, canvas.height);

          // Draw PNG image over the white background
          ctx.drawImage(img, 0, 0);

          // Convert to JPEG blob with custom quality
          canvas.toBlob((blob) => {
            if (blob) {
  ...
  const handleDownload = () => {
    if (convertedUrl && file) {
      ...
      saveAs(convertedUrl, `${originalName}.jpg`);
    }
  };
  ...
  const toolFAQs = [
    {
      question: 'How are transparent backgrounds handled?',
      answer: 'Since the JPG format does not support transparency, our converter automatically fills all transparent areas with a clean, high-contrast solid white color background. This prevents black silhouette distortions.'
    },
    {
      question: 'What does the Quality Slider do?',
      answer: 'JPEG uses lossy compression. The quality slider determines the balance between file size and detail level. A value of 90% is highly recommended as it keeps the image pristine while heavily compressing file size.'
    },
    {
      question: 'Where are my images processed?',
      answer: 'Everything executes inside your browser using canvas rendering. No information, files, or pixels are uploaded to any server.'
    }
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <ToolHeader
        ...
        description="Convert PNG images into highly compressed, high-quality JPG files with custom quality sliders."
          ...
          <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-900 p-6 rounded-2xl shadow-sm">
            {!file ? (
              <FileDropzone
                ...
                descriptionText="Upload a PNG image to convert"
              />
            ) : (
              <div className="space-y-6">
                ...
                {error && (
                  <p id="png-to-jpg-error" className="text-sm text-red-600 font-semibold mt-2 text-center">
                    {error}
                  </p>
                )}
              </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-950/20 p-5 rounded-2xl border border-gray-150 dark:border-gray-900 space-y-4">
            ...
            <ul className="space-y-2.5 text-xs text-gray-600 dark:text-gray-400 font-medium">
              ...
              <li className="flex justify-between border-b border-gray-100 dark:border-gray-900 pb-2">
                ...
                <span className="font-bold text-gray-950 dark:text-white">Auto-filled White</span>
              </li>
